...
class openFolder:
    ...
    def init_folder_list(self):
        self.folder_list = [name for name in os.listdir(self.root_path) if os.path.isdir(os.path.join(self.root_path, name))]
    ...
    def create_folder(self):
        for folder in self.folder_list :
            try : 
                ...
                print('Error: ' + self.root_path + folder + '\\새폴더명1\\' + '폴더 생성에 실패했습니다.')

            try : 
                if not os.path.exists(self.root_path + folder + '\\새폴더명2\\') :
                    os.makedirs(self.root_path + folder + '\\새폴더명2\\')
            except OSError :
                print('Error: ' + self.root_path + folder + '\\새폴더명2\\' + '폴더 생성에 실패했습니다.')
